...
## 트랜잭션 전파 - 다양한 옵션
스프링은 다양한 트랜잭션 전파 옵션을 제공한다. 하지만 대부분 기본 옵션인 `REQUIRED`를 사용하고 아주 가끔 `REQUIRES_NEW`를 사용한다.
나머지는 거의 사용하지 않는다. 그래서 나머지 옵션은 이런 것이 있따는 정도로만 알아두고 필요할 때 찾아보자.

- REQUIRED: 외부 트랜잭션이 있으면 참여하고, 없으면 새로운 트랜잭션을 생성한다. (default)
- REQUIRES_NEW: 항상 새로운 트랜잭션을 생성하고, 기존 트랜잭션은 일시 정지한다.
- SUPPORT: 외부 트랜잭션이 있으면 참여하고, 없으면 트랜잭션 없이 동작한다.
  - 기존 트랜잭션 없음: 트랜잭션 없이 진행
  - 기존 트랜잭션 있음: 트랜잭션 참여
- NOT_SUPPORTED: 트랜잭션 없이 동작하고, 기존 트랜잭션은 일시 정지한다.
- MANDATORY: 트랜잭션이 반드시 있어야 한다. 기존 트랜잭션이 없으면 예외가 발생한다.
- NEVER: 트랜잭션 없이 동작해야 한다. 기존 트랜잭션이 있으면 예외가 발생한다.
- NESTED
  - 기존 트랜잭션 없음: 새로운 트랙잭션을 생성한다.
  - 기존 트랜잭션 있음: 중첩된 트랜잭션을 생성한다.
